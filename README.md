...
- A reproducible captured-domain experiment

A two-epoch, CPU-only pipeline smoke run validated the machinery but remains explicitly non-reportable. The first full RadioML-based baseline is now complete and reported below.
...
The experiment uses a deterministic 9,216-window subset of RadioML 2018.01A: all 24 modulation classes, source SNR levels `[-20, -10, 0, 10, 20, 30]` dB, and exactly 64 source windows per modulation/SNR cell. The full RadioML dataset is not redistributed.

## Synthetic Jamming Task

The repository defines `clean = 0` and `jammed = 1`. Jammed examples contain a deterministic synthetic tone, chirp, or barrage-noise jammer at JSR `[-10, -5, 0, 5, 10]` dB. These binary labels and jammer labels are generated by this repository; they are not native RadioML labels.

`SNR` always means the original RadioML signal condition. `JSR` means the jammer-to-signal power ratio introduced by this project.

## Baseline Model

The current provisional compact CNN has exactly `240,962` trainable parameters. It is an honest first baseline, not a recovered historical architecture and not the result of a hyperparameter search.
...
The first full baseline used seed 42, a modulation/source-SNR-stratified 6,451/1,382/1,383 source-window split, Adam at learning rate 0.001, batch size 128, and early stopping by validation loss. The best checkpoint was epoch 7. The balanced test contains one clean and one deterministic jammed example per held-out source (2,766 examples).

| Balanced held-out metric | Result |
| --- | ---: |
| Accuracy | 87.02% |
| Jammed precision | 100.00% |
| Jammed recall / detection rate | 74.04% |
| Jammed F1 | 85.09% |
| Macro F1 | 86.80% |
| Specificity | 100.00% |
| Clean false-positive rate | 0.00% |

The separate stress test applies all 15 jammer/JSR variants to each of the 1,383 test sources (20,745 jammed examples). It is never used for training or checkpoint selection.

| JSR (dB) | Tone DR | Chirp DR | Barrage DR | Overall DR |
| ---: | ---: | ---: | ---: | ---: |
| -10 | 6.87% | 5.71% | 4.84% | 5.81% |
| -5 | 66.74% | 64.28% | 53.36% | 61.46% |
| 0 | 99.93% | 100.00% | 100.00% | 99.98% |
| 5 | 100.00% | 100.00% | 100.00% | 100.00% |
| 10 | 100.00% | 100.00% | 100.00% | 100.00% |

The complete run, including checkpoint, source indices, predictions, detailed modulation/SNR/jammer/JSR results, sanity audits, and unsmoothed plots, is in [`runs/20260909T001620Z_baseline`](runs/20260909T001620Z_baseline). The result shows an important limitation hidden by overall accuracy: weak negative-JSR jammers, especially barrage noise, are often missed.

These numbers belong to the new reproducible RadioML-based experiment. They do not reproduce or validate numerical claims from the unrecovered historical RF experiment.
...
- [x] Inspect and validate the actual RadioML 2018.01A file
- [ ] Recover or document differences from the original architecture
- [x] Run and report a held-out source-domain baseline
